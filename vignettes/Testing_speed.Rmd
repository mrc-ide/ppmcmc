...
1. Row-wise, using `add_to_rowfirst()` or
2. Column-wise using `add_to_colfirst()`.
...
add_to_row_first(x, 0, 1)
x

add_to_col_first(x, 0, 1)
x
```
...
test_row_first <- function(n){
  x <- matrix(0L, nrow = n, ncol = n)
  add_to_row_first(x, 0, 1)
}
test_col_first <- function(n){
  x <- matrix(0L, nrow = n, ncol = n)
  add_to_col_first(x, 0, 1)
}
row_v_col <- microbenchmark(test_row_first(1000), test_col_first(1000), times = 100)
ggplot2::autoplot(row_v_col)
```
We can see that, as the  elements of the matrix are stored in the vector in memory
in column-major order, operating column-wise is quicker.


We can also show that is more inefficient if a copy is forced to be made if we 
pass a numeric matrix (not integer):

```{r, message=FALSE, fig.width=10}
test_col_first_copy <- function(n){
  x <- matrix(0, nrow = n, ncol = n)
  add_to_col_first(x, 0, 1)
}
col_v_copy <- microbenchmark(test_col_first(1000), test_col_first_copy(1000), times = 100)
